# Оптимизация прибыльности аптечной сети   
"Анализ продаж аптечной сети: проверка гипотез"   

## Содержание
- [Описание проекта](#описание-проекта)
- [Данные](#данные)
- [Саммари проекта](#саммари-проекта)
- [Гипотезы и результаты](#гипотезы-и-результаты)
- [Отчет](#отчет)
    - [Гипотеза №1](#гипотеза-1)
    - [Гипотеза №2](#гипотеза-2)
    - [Гипотеза №3](#гипотеза-3)
    - [Гипотеза №4](#гипотеза-4)
    - [Гипотеза №5](#гипотеза-5)
    - [Гипотеза №6](#гипотеза-6)
    - [Гипотеза №7](#гипотеза-7)
    - [Гипотеза №8](#гипотеза-8)
    - [Гипотеза №9](#гипотеза-9)
    - [Гипотеза №10](#гипотеза-10)
- [Дальнейшие шаги](#дальнейшие-шаги)
- [Допущения и оговорки](#допущения-и-оговорки)
- [Техническая информация о проекте](#техническая-информация-о-проекте)
...
Проект посвящён исследованию факторов, влияющих на продажи аптечной сети, с применением статистического тестирования гипотез и бизнес-интерпретации результатов.
Основная цель — выявить закономерности, определяющие различия в среднем чеке, прибыли и эффективности поставщиков, аптек и сотрудников, а также оценить значимость управляемых факторов (скидки, форма оплаты, тип закупки и т.д.).  

В ходе анализа реализован полный цикл статистического исследования:  

- **Подготовка данных** (загрузка из SQL-базы с применением ООП-класса-обёртки, очистка и агрегация);  
- **Разведочный анализ данных (EDA)** — визуализация распределений, оценка нормальности и выбросов;  
- **Проверка гипотез** — применение параметрических и непараметрических методов (Welch’s t-test, Mann–Whitney U, Kruskal–Wallis, χ² и Monte Carlo χ²);  
- **Пост-хок анализ** — сравнение групп с коррекцией множественных сравнений (Holm, FDR);  
- **Оценка размера эффекта** (Cohen’s d, Cliff’s δ, η²/ε², Rank-biserial) для количественной интерпретации различий;  
- **Кластеризация поставщиков** по структуре прибыльности (KMeans + StandardScaler);  
- **Интерпретация бизнес-значимости** — формулирование выводов и управленческих рекомендаций.  

Результаты демонстрируют, что ключевыми драйверами вариации показателей являются тип закупки, политика скидок и различия между поставщиками.
Аптеки и сотрудники вносят сопоставимый вклад в средний чек, что подтверждает необходимость учитывать контекст работы при оценке персональной эффективности.
Проект показывает, как классические статистические методы могут использоваться для построения прогностических признаков и принятия решений на основе данных.  
...
| 10 | **Зависимость между поставщиком и категорией прибыльности**  <br> H₀: Категория прибыльности не зависит от поставщика. <br> H₁: Категория прибыльности зависит от поставщика. | H₀ отвергнута     | [hypothesis_10.ipynb](https://github.com/Advantl/Statistics_project/blob/main/notebooks/hypothesis_10.ipynb)  |

[↑ Вернуться к содержанию](#содержание)

## Отчет 
В отчете применяются следующие обозначения для статистических тестов:  
...
В рамках этапа подготовки данных были удалены записи, содержащие технические ошибки или аномальные значения, которые могли повлиять на корректность анализа.  
- Удалены строки, в которых сумма скидки ('dr_sdisc') и количество проданного товара ('dr_kol') принимают отрицательные значения.  
- Исключены три чека с идентификаторами 21, 593 и 2495, сумма которых значительно превышет общую ценовую динамику выборки (от 18 000 до 35 000 руб.), при этом максимальное значение по остальным чекам не превышает 13 000 руб. Особенно выделяется чек 593, содержащий единственную продажу дорогостоящего лекарственного средства стоимостью более 18 000 руб., которая была зафиксирована лишь один раз за весь период наблюдения. Данные чеки были исключены из анализа, поскольку представляют собой ценовые выбросы, не отражающие типичное поведение потребителей, и могли бы существенно исказить результаты статистического анализа. В случае оставшихся чеков с аномально высокой стоимостью было принято допущение, что данные значения отражают формы крупных «натуральных» продаж и не являются результатом ошибок ввода или искажения данных.  
   
**Разведочный анализ данных (EDA)**  

В рамках разведочного анализа данных (EDA) был проведён комплексный обзор ключевых признаков, участвующих в проверке статистических гипотез, с целью выявления закономерностей, особенностей распределений и потенциальных аномалий. Анализ включал как визуальные, так и числовые методы описания данных.  
Для визуализации использовались различные графические инструменты библиотеки seaborn и matplotlib, включая:  
- *histplot* и *kdeplot* — для оценки формы распределений и проверки их на симметричность, наличие выбросов и отклонений от нормальности;  
- *probplot* — для дополнительной проверки нормальности признаков на основе квантиль-квантильных графиков;  
- *boxplot* и *violinplot* — для анализа медианных значений, межквартильных размахов и плотности распределения показателей;  
- *scatterplot* и *heatmap* — для изучения возможных взаимосвязей между количественными переменными и визуализации корреляционной структуры;  
- *barplot* — для сравнения категориальных групп по средним и медианным значениям.
  
Помимо визуализации, были рассчитаны основные описательные статистики (среднее, медиана, стандартное отклонение, минимум, максимум, квартили), что позволило количественно охарактеризовать распределения и оценить их вариабельность.

Результаты EDA легли в основу выбора подходящих статистических методов для проверки гипотез. В частности, оценка нормальности распределений и однородности дисперсий позволила определить, в каких случаях целесообразно применение параметрических тестов (например, Welch’s t-test, ANOVA), а где — непараметрических методов (Mann–Whitney U test, Kruskal–Wallis H-test). Таким образом, этап EDA обеспечил корректность последующего статистического анализа и способствовал повышению достоверности выводов по исследуемым гипотезам.   

**Интерпретация метрик размера эффекта**  

Размер эффекта (*Effect Size*) — количественная мера силы различий или связей, дополняющая p-значения.  
В отличие от p-value, он отражает насколько значимы различия, а не просто факт их наличия.  

| **Метрика**                           | **Тип данных / тест**              | **Малый эффект** | **Средний эффект**  | **Крупный эффект**                  | **Источник / комментарий**             |
| ------------------------------------- | ---------------------------------- | ---------------- | ------------------- | ----------------------------------- | -------------------------------------- |
| **Cliff’s Delta (δ)**                 | Непараметрическая (2 группы)       | \|δ\| < 0.147      | 0.147 ≤ \|δ\| < 0.33  | \|δ\| ≥ 0.474                         | Romano et al., 2006                    |
| **Cohen’s d**                         | Параметрическая (t-тест, ANOVA)    | \|d\| < 0.2        | 0.2 ≤ \|d\| < 0.5     | \|d\| ≥ 0.8                           | Cohen, 1988                            |
| **Eta-squared (η²)**                  | ANOVA (параметрическая)            | η² < 0.01        | 0.01 ≤ η² < 0.06    | η² ≥ 0.14                           | Cohen, 1988                            |
| **Epsilon-squared (ε²)**              | Kruskal–Wallis (непараметрическая) | ε² < 0.01        | 0.01 ≤ ε² < 0.06    | ε² ≥ 0.14                           | Аналог η², скорректированная версия    |
| **Rank-biserial correlation (r₍rb₎)** | Непараметрическая (Mann–Whitney U) | \|r₍rb₎\| < 0.1    | 0.1 ≤ \|r₍rb₎\| < 0.3 | \|r₍rb₎\| ≥ 0.5                       | Альтернатива r Пирсона                 |

Eta-squared (η²<sub>H</sub>) и Epsilon-squared (ε²<sub>H</sub>) использовались как адаптированные непараметрические меры размера эффекта для Kruskal–Wallis H-теста. Они показывают долю вариации рангов, объясняемую фактором (например, различиями между поставщиками). Интерпретация проводится по шкале Cohen (1988): 0.01 — малый, 0.06 — средний, 0.14 — большой эффект (Tomczak & Tomczak, 2014).  
Данные адаптации предложены и описаны в:    
- Tomczak & Tomczak (2014) — *"The need to report effect size estimates revisited"*  
(Trends in Sport Sciences, 21(1), 19–25).  
- Lehmann, 2006; Fritz, Morris & Richler, 2012 — рассматривают применение размеров эффекта для непараметрических тестов.

Эти работы закрепили использование η²<sub>H</sub> и ε²<sub>H</sub> как стандартных непараметрических оценок силы эффекта при Kruskal–Wallis.  

[Источник](#допущения-и-оговорки)  

**Методы коррекции в пост-хок анализе**  

При множественных попарных сравнениях существенно возрастает риск ошибки первого рода (ложного отклонения нулевой гипотезы).  
Цель коррекции — снизить общий риск ложных значимых результатов (контроль за FWER или FDR).  
*FWER = Family-Wise Error Rate* — вероятность хотя бы одной ошибки I рода.  
*FDR = False Discovery Rate* — доля ложноположительных среди всех значимых.  

| Метод                        | Контролирует | Консервативность     | Использование                                                          |
| ---------------------------- | ------------ | -------------------- | ---------------------------------------------------------------------- |
| **Bonferroni**               | FWER         | Очень высокая        | Малое число сравнений, строгий контроль                                |
| **Holm–Bonferroni**          | FWER         | Средняя              | Предпочтительнее Bonferroni                                            |
| **Holm**                     | FWER         | Средняя              | Универсальный метод: подходит при любом числе сравнений, рекомендуется вместо Bonferroni |
| **Benjamini-Hochberg (BH)**  | FDR          | Гибкий, умеренный    | Большое число сравнений.                                               |
...
•	Поставщики «Пульс», «Фармкомплект ООО», «Протек», «ООО Континент», а также агрегированная группа «Другие» показали долю значимых различий выше 0.8.  
...
•	Визуализация по средней прибыли и объёму продаж выявила отдельный кластер крупных и высокоприбыльных поставщиков, а также кластер низкомаржинальных поставщиков, характерных для розничных аптечных сетей.  
•	При учёте вклада в общую выручку видно, что топ-3 поставщика обеспечивают непропорционально большую долю прибыли, формируя зависимость бизнеса от ограниченного числа партнёров.  

*«Матрица эффективности поставщиков (объём продаж × средняя прибыль)»*

![Матрица поставщиков с учетом вклада в общую выручку](visualizations/scatter_supplier_grouped.png) 

Двумерная визуализация, отражающая взаимосвязь между объёмом продаж и средней прибылью по каждому поставщику.  
Каждая точка на графике представляет отдельного поставщика:  
— положение по оси X соответствует количеству продаж,  
— положение по оси Y — средней прибыли на единицу продажи,  
— размер маркера обозначает совокупную выручку.  

Такой формат позволяет выявить поставщиков с высокой рентабельностью при умеренных объёмах (эффективные ниши), а также крупных контрагентов с низкой маржой, требующих пересмотра условий сотрудничества.
Визуализация дополняет результаты статистических тестов и способствует интерпретации различий в прибыльности, выявленных в ходе анализа.
...
|----------------------|------------------------------------------|---------------------------------------------|---------------------------------------------------------------------------|  
| I (право-верхний)    | Высокий объём и высокая средняя прибыль  | Поставщики приносят основную прибыль        | Лидеры — приоритетное сотрудничество, условия сохранить                   |  
| II (лево-верхний)    | Низкий объём, но высокая средняя прибыль | Товары дорогие, но продаж немного           | Перспективные — можно развивать продажи, расширять ассортимент            |  
| III (право-нижний)   | Высокий объём, но низкая средняя прибыль | Массовый поток, но низкая маржа             | Поддержка — нужны для оборота, но стоит оптимизировать закупочные цены    |  
| IV (лево-нижний)     | Низкий объём и низкая средняя прибыль    | Почти не дают прибыли                       | Слабые — кандидаты на пересмотр или исключение                            |  
...
4. Корректировка p-значений (Holm, FDR) сильно подняла значения, что указывает на множество сравнений и небольшие выборки, повышающие риск случайных различий.
...
**Значимость проверки гипотезы для бизнеса:**   
Изучение зависимости между поставщиком и категорией прибыльности имеет ключевое значение для управления ассортиментом и оптимизации закупочной стратегии аптечной сети. Если категория прибыльности статистически зависит от поставщика, это означает, что определённые поставщики систематически связаны с товарами более высокой или низкой маржинальности. Такая информация позволяет компании сегментировать партнёров по уровню финансовой эффективности, пересматривать условия сотрудничества и формировать приоритетные закупочные списки. Кроме того, выявление поставщиков, ассоциированных с низкомаржинальными или убыточными категориями, помогает точечно корректировать ассортимент, пересмотреть скидки или изменять объём закупок. В результате бизнес получает возможность повысить общую прибыльность товарного портфеля, сократить издержки и улучшить структуру продаж за счёт данных-обоснованных решений в управлении поставками.  
...
Для проверки гипотезы была сформирована таблица сопряжённости, отражающая распределение количества продаж по трём категориям прибыльности («низкая», «средняя», «высокая») в разрезе поставщиков. Применение критерия χ² показало наличие статистически значимой зависимости между поставщиком и категорией прибыльности (χ² = 1215.28, p < 0.001, dof = 38). Минимальное ожидаемое значение ячейки составило 10.997, что подтверждает корректность применённого критерия.  
Для проверки устойчивости результата был проведён перестановочный (Monte Carlo) χ²-тест на 5000 итераций, который также подтвердил высокую значимость эффекта (эмпирическое p-value ≈ 0.00000).  

Пост-хок анализ (перестановочные тесты для всех пар поставщиков с FDR-коррекцией) выявил многочисленные значимые различия в профилях прибыльности между поставщиками. Особенно выраженные отличия наблюдаются у компаний «Пульс», «Фармкомплект ООО», «Протек», «Катрен г. Химки», «ООО Континент» и «ГРАНД КАПИТАЛ СМОЛЕНСК ООО ФК», которые систематически демонстрируют нетипичное распределение категорий прибыльности относительно остальных участников. Это указывает на различие стратегий ценообразования, скидочной политики и товарного ассортимента у данных поставщиков.  

Дополнительно, с целью углублённого анализа выявленной зависимости между поставщиком и категорией прибыльности, был проведён кластерный анализ поставщиков. Для этого использовался метод KMeans, применённый к матрице профилей прибыльности, представляющей доли низкоприбыльных, среднеприбыльных и высокоприбыльных товаров по каждому поставщику. Перед кластеризацией данные были стандартизированы с помощью StandardScaler для устранения влияния различий в масштабах признаков. Оптимальное число кластеров было определено на основе метода «локтя», что позволило выбрать значение k = 5 как обеспечивающее баланс между объяснённой дисперсией и устойчивостью сегментов.  
В результате поставщики были разделены на пять устойчивых кластеров, различающихся по структуре прибыльности. Для каждого кластера был рассчитан усреднённый профиль распределения долей прибыльности, что позволило выделить сегменты с преобладанием низкой, средней или высокой прибыльности, а также смешанные типы с более равномерным распределением категорий. Визуализация в виде стековой диаграммы продемонстрировала чёткие различия в структуре прибыльности между кластерами, подтверждая неоднородность поставщиков по данному признаку.  

Полученные результаты сегментации дополняют результаты χ²-теста и пост-хок анализа, подтверждая, что поставщики действительно образуют группы с различными стратегическими профилями прибыльности. Эти кластеры могут использоваться как категориальный признак (например, supplier_cluster) в прогнозных моделях или как основа для бизнес-стратегий — например, при разработке индивидуальных условий сотрудничества, оптимизации закупок и управлении ассортиментом.  

*Структура прибыльности по кластерам поставщиков*  
  
![Структура прибыльности по кластерам поставщиков](visualizations/bar_profit_profile.png)

*Средний профиль по кластерам*  
| Кластер | Низкая прибыльность | Средняя прибыльность | Высокая прибыльность |  
| ------- | ------------------- | -------------------- | -------------------- |  
...
*Управленческое действие:* Поддерживать долгосрочные отношения, контролировать условия, но без необходимости жёстких переговоров.   
...
В рамках данного исследования были протестированы ключевые гипотезы о различиях и зависимостях, связанных с прибылью продаж, поставщиками, аптечными подразделениями и категориями товаров. Однако для углубления аналитики и повышения прикладной ценности результатов представляется целесообразным провести дополнительные исследования и проверки гипотез:  

1. Анализ сезонности и временной динамики прибыльности.  
Проверить наличие статистически значимых различий прибыльности по месяцам, кварталам и сезонам. Это позволит оценить устойчивость результатов по времени и выявить периоды пиковых продаж или падений.  

2. Зависимость прибыльности от географических факторов.  
В случае наличия данных о локации аптек — проверить, различается ли структура прибыльности в зависимости от района, формата (город/пригород), уровня покупательской активности и локальных предпочтений.   

3. Влияние способа оплаты и канала продаж на прибыльность.   
Провести факторный анализ или множественную регрессию, чтобы количественно оценить вклад каждого фактора (например, тип оплаты, онлайн/офлайн канал, поставщик) в общую прибыльность.  

4. Взаимосвязь между объёмом заказов и маржинальностью.  
Проверить, существует ли корреляция между количеством заказов по аптеке или поставщику и средней прибылью на заказ. Это поможет выявить, какие поставщики обеспечивают не только объём, но и качественную (прибыльную) нагрузку.  

5. Оценка стабильности поставщиков во времени.  
Проверить, сохраняются ли выявленные различия между поставщиками на разных временных интервалах (например, по месяцам или кварталам). Возможно, кластеризация поставщиков по прибыльности изменяется во времени, что требует адаптивных моделей.  

6. Анализ состава ассортимента в контексте прибыльности.  
Проверить гипотезу о том, что ассортиментная широта или доля определённых категорий (например, Rx/OTC, косметика, БАДы) связана с общей прибылью аптеки или поставщика.  

7. Построение прогнозных моделей прибыльности.  
Использовать выявленные закономерности (например, принадлежность к кластеру поставщика, категория прибыльности, доля скидочных продаж) как признаки (features) в моделях машинного обучения для прогнозирования будущей прибыльности продаж.  

8. Проверка репрезентативности данных и чувствительности результатов.  
На следующем этапе — оценить репрезентативность выборки по аптекам и временным периодам, провести бутстреп-анализ или перекрёстную валидацию, чтобы оценить устойчивость статистических выводов.  

9. Анализ взаимодействий факторов (многофакторные модели).  
Например, проверить наличие взаимодействия между поставщиком и категорией товара в контексте прибыльности — влияет ли один фактор на другой, или эффект является независимым.  

10. Расширение кластерного анализа.  
Провести повторную кластеризацию с учётом дополнительных признаков (например, средний чек, частота поставок и т.д.), что позволит выделить более устойчивые профили поставщиков.  
...
1. Источники статистических шкал и интерпретаций  
   
Для интерпретации размера эффекта (effect size) использовались общепринятые шкалы из классических статистических источников. Это позволяет оценивать не только наличие статистической значимости (p-value), но и практическую силу обнаруженных различий.
 
**Cohen (1988) — Statistical Power Analysis for the Behavioral Sciences**    
> Jacob Cohen, профессор Нью-Йоркского университета, систематизировал понятие размера эффекта и предложил стандартные шкалы интерпретации.  
> Его работа заложила основу современной аналитической практики в психологии, экономике и прикладной статистике.

Используемые метрики и шкалы:  
- Cohen’s d — стандартизированная разница между средними  
        - Малый эффект: 0.2  
        - Средний эффект: 0.5  
        - Крупный эффект: 0.8
  
- Eta-squared (η²) и Epsilon-squared (ε²) — доля объяснённой дисперсии в ANOVA  
        - Малый: 0.01  
        - Средний: 0.06   
        - Крупный: 0.14
  
- Cramér’s V — сила связи между категориальными переменными  
        - Малый: 0.1  
        - Средний: 0.3  
        - Крупный: 0.5
...
Используемые метрики и шкалы:  
- Cliff’s Delta (δ) — вероятность того, что случайное наблюдение из одной группы больше, чем из другой  
        - Незначительный эффект: |δ| < 0.147  
        - Малый: 0.147 ≤ |δ| < 0.33  
        - Средний: 0.33 ≤ |δ| < 0.474  
        - Крупный: |δ| ≥ 0.474
...
2. Рекомендации по стандартизации и подготовке данных.  
...
**Ядро**  
- Python 3.9+ — основной язык (в репозитории указан совместимый минимальный релиз).  
- Jupyter Notebook / JupyterLab — интерактивная аналитика, визуализация и отчётность.
   
**Работа с данными**    
- pandas — подготовка данных, агрегирование (группировки по чекам/аптекам/поставщикам), создание таблиц сопряжённости.  
- numpy — численные операции и эффективные массивы.
  
**Визуализация и отчётность**    
- matplotlib, seaborn — графики: histplot, kdeplot, boxplot, violinplot, scatterplot, heatmap, stacked barplot.
  
**Статистика и проверка гипотез**    
- scipy.stats — t-test, Mann–Whitney U, Kruskal–Wallis, Levene, Kolmogorov–Smirnov и др.  
- statsmodels — дополнительные статистические тесты (proportions_ztest, OLS, MixedLM), поправки для множественных сравнений.  
- pingouin — дополнительные непараметрические/ранговые методы и effect-size.  
- scikit-posthocs — пост-хок тесты (Dunn и пр.).  
- custom / permutation — перестановочные тесты (Monte-Carlo χ², парные permutation-тесты), реализованы вручную для устойчивости при малых ожидаемых частотах.
  
**Размер эффекта и интерпретация**    
- Реализовано: Cliff’s Delta, Cohen’s d, Eta-squared / Epsilon-squared, Rank-biserial — комплексная интерпретация статистических различий и дополнение традиционного анализа на основе p-значений.
  
**Машинное обучение и сегментация**    
- scikit-learn:  
        - KMeans — кластеризация профилей поставщиков по структуре прибыли;  
        - StandardScaler — обязательное масштабирование признаков перед KMeans;  
        - Подход: профилирование (квантильное разбиение прибыли с целью получить профили поставщиков), с последующей кластеризацией (KMeans на стандартизированных долях).
  
**Модуль доступа и интеграции с базой данных (OOP-реализация)**     
...
В проекте реализовано простое файловое кэширование SQL-запросов для ускорения аналитических операций и снижения нагрузки на базу данных. Механизм встроен непосредственно в метод execute_query() класса PostgresConnection.  
